import {
    ...
    VampireBlast,
    XMLObject
} from "@rotmg-mirror/rotmg-utils"
import { AssetTypes, Manager } from "../asset"
import {
    getEquipmentFromState,
    getPlayerFromState,
    hasStatusEffect,
    Item,
    PlayerState,
...
const EnemyEffects = [StatusEffectType["Armor Broken"], StatusEffectType.Curse, StatusEffectType.Exposed]
...
function getAverageDamage(
    ...
    if (data.armorPiercing) {
        ...
        for (let i = data.minDamage; i < data.maxDamage - 1; i++) {
            ...
            let localDamage = Math.max(base - def, base * 0.1)
            if (enemyEffects.hasEffect(StatusEffectType.Curse)) {
                localDamage *= 1.25
            }
            damage += localDamage
...
function processProcs(
    key: ProcNames,
    data: ProcDatas,
    items: PossibleItem[],
    equipment: (Equipment | undefined)[],
    options: DPSProviderOptions
) {
...
export default class DPSCalculator {
    ...
    getProviders(): DPSProvider[] {
        return [new WeaponDPSProvider(this.state), new AbilityDPSProvider(this.state)]
    }
...
class WeaponDPSProvider implements DPSProvider {
    ...
    simulate(options: DPSProviderOptions): boolean {
        ...
        const attacks =
            weapon.subAttacks.length <= 0
                ? [{ ...weapon, projectileId: 0 }]
                : weapon.subAttacks.map(attack => {
        ...
        if (!options.playerEffects.hasEffect(StatusEffectType.Stunned))
            for (const i in attacks) {
                ...
                while (this.attackCountsBuffer[i] >= 1) {
                    ...
                    this.dps +=
                        (damage *
                            (attack.numProjectiles ?? weapon.numProjectiles ?? 1) *
                            (this.state.equipment[0]?.accuracy ?? 100)) /
                        100
                    ...
                    if (
                        projectile.conditionEffect !== undefined &&
                        projectile.conditionEffect.type === StatusEffectType.Bleeding
                    ) {
    ...
    getAttacksPerSecond(weapon: WeaponAttack, stats: Stats, effects: StatusEffectManager): number {
        ...
        const currBurstDelay = Math.min(
            Math.max(weapon.burstDelay - ((weapon.burstDelay - weapon.burstMinDelay) / 100) * stats.dex, weapon.burstMinDelay),
            weapon.burstDelay
        )
...
class AbilityDPSProvider implements DPSProvider {
    ...
    simulate(data: DPSProviderOptions): boolean {
        ...
        while (this.abilityUses > 0 && this.mana > this.mpCost) {
            ...
            this.abilityUses--
            if (data.playerCreeps.getCreeps().length !== 3) this.mana -= this.mpCost
        }
...
class CreepProvider extends TimedActivateProvider<SpawnCreep> {
    ...
    private _duration = Number(
        this.equip.extraTooltipData.find(x => x.name === "Lifetime")?.description.replace("seconds", "") ?? this.equip.cooldown
    )
...
const ObjectTossProviders: {
    [key: string]: new (item: PossibleItem, equip: Equipment, proc: any) => ActivateProvider<ObjectToss>
} = {
